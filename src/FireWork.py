...
def bashFile(formattedParticuleString, selectedDate):
    ...
    for modelHourSeparated in modelHourList:
        ...
        fileBash.write(
            ...
            "\nListeEspeces=\""+formattedParticuleString+"\"" # formattedParticuleString
...
def sortAndGenerate(destination, selectedDate):
    ...
    for m in modelHourList:
        for p in particulelist:
            uniqueName = uniquify("output_csv/"+year + month + day +m+"_"+"FireWork"+"_"+p+"_"+Gm.returnName(locationId)+".csv")
            ...
            file.close()
            Gm.generateExcel(uniqueName.split("/")[-1])
